...
As for causality, an ideal goal might be to only place edges if perturbing one element necessarily *causes* a change in the downstream element. We will return back to this when we discuss CRISPR knockouts in [Chapter 7](chapter7_crispr.qmd). However, suffice to say, for now, this is extremely challenging for many aspects:
