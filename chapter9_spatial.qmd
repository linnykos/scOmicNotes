...
::: block
![Left: From @kim2023single](fig/chap9/visium-spots2.png){#fig-visium-spots1 width="100%" fig-align="center"}
:::

::: block
![Right: From <https://satijalab.org/seurat/articles/spatial_vignette>](fig/chap9/visium-spots.png){#fig-visium-spots2 width="100%" fig-align="center"}
:::
...
-   **Optimal transport for a single ligand-receptor pair**: Given a ligand $i$ and receptor $j$, COMMOT formulates the inference of CCC as an optimal transport problem. The goal is to determine the transport plan $P_{\ell,r} \in \mathbb{R}^{n\times n}_+$ for $n$ cells, where $[P_{\ell,r}]_{k,l}$ quantifies the strength of signaling from sender cell $i$ to receiver cell $j$ through the ligand-receptor pair $(\ell,r)$. This is achieved by minimizing the transport cost:
    ...
    where $X_{i,\ell}$ and $X_{j,r}$ represent the expression levels of the ligand and receptor in the respective cells.[^chapter9_spatial-1]
...
[^chapter9_spatial-1]: The first constraint $\sum_{j} [P_{\ell,r}]_{i,j} \leq X_{i,\ell}$ says, "Given that cell $i$ has this much expression of ligand $\ell$, it should not be sending more than $X_{i,\ell}$ to all the other $n$ cells." The second constraint $\sum_{i} [P_{\ell,r}]_{i,j} \leq X_{j,r}$ says, "Given that cell $j$ has this much expression of receptor $r$, it should not be receiving more than $X_{j,r}$ from all the other $n$ cells."
