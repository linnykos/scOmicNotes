...
::: block
![Histogram of percentage of cells with non-zero counts for each gene of the dataset shown in @fig-count1](fig/chap2/nonzero_hist.png){#fig-count2 width="100%" fig-align="center"}
:::
...
(This step will make more sense once you know a bit about how scRNA-seq data is generated, see [Chapter 3](chapter3_rna.qmd).)
...
::: block2
**Remark (The term "batch" is very overloaded).** There are many steps during a single-cell analysis (see [Chapter 3](chapter3_rna.qmd)). These are some of the things a "batch" could refer to: 1) cells processed together during droplet/gem formation, 2) cells processed during during PCR amplification, 3) cells processed together during sequencing (this is most common usage of the word "batch"), 4) cells processed using different protocols, 5) cells processed by different labs, 6) cells of different donors/organisms, among many more. (The first three steps are similar in "resolution," but the last three are more "coarse" in how a batch is defined.)
