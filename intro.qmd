...
**Causality**: cCmes from a chain of experiments. Suppose we study a gene’s role in disease:
...
::: block
-   **Biological context** – What system and “north-star” question? Which premises are accepted, which are tested, and why does this mechanism matter?
-   **Technology, experiment, data** – How are data collected and why this *{technology, experiment, data}* trio? What technical artefacts arise?
-   **Boundaries of current tools** – Simple analyses first: what breaks? Is there preliminary evidence a new method would do better?
-   **Statistical model** – What insight can better interrogate the biology? (Here the statistics training begins.)
-   **Develop a method & show robustness** – Numerically (noise tolerance) and biologically (applicable across contexts). Often biology lacks a ground truth, so validity arguments lean on biological logic.
-   **Uncover new / refined biology** – Does the method advance understanding? How confident are we that findings generalise beyond the original *{technology, experiment, data}* trio? (Usually the paper’s highlight.)
:::
